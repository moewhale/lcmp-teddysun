...
EOF
    ...
    if [[ -f "${SCRIPT_DIR}/conf/lcmp.png" ]]; then
        _error_detect "cp -f ${SCRIPT_DIR}/conf/lcmp.png /data/www/default/"
    fi
...
_install_components() {
    ...
    if [[ -f "${SCRIPT_DIR}/conf/lcmp" ]]; then
        _error_detect "cp -f ${SCRIPT_DIR}/conf/lcmp /usr/bin/"
        _error_detect "chmod +x /usr/bin/lcmp"
    fi
