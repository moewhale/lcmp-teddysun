...
# 使用 | 作为分隔符，代码更清晰
sed -i 's|/repo/mariadb-server|/browse/mariadb_server|g' mariadb_repo_setup.sh
...
if check_sys rhel; then
    ...
    _error_detect "apt-get update"
    _error_detect "apt-get install -y php-common php${php_ver}-common php${php_ver}-cli php${php_ver}-fpm php${php_ver}-opcache php${php_ver}-readline --ignore-missing || true"
    ...
    _error_detect "apt-get install -y php${php_ver}-tidy php${php_ver}-sqlite3 php${php_ver}-imagick php${php_ver}-grpc php${php_ver}-yaml php${php_ver}-uuid --ignore-missing || true"
    _error_detect "mkdir -p -m770 /var/lib/php/{session,wsdlcache,opcache}"
fi
